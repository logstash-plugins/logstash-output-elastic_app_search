...
Engine field also support {logstash-ref}/event-dependent-configuration.html#sprintf[sprintf format], to enable
the engine name to be derived from a field value from each event, for example `engine-%{engine_name}`.

Caution:
Invalid engine names will cause ingestion to be halted until the field value can be resolved into a valid engine name.
This be caused by either the field value being interpolated resolving to a value without a matching engine,
or, if the field is missing from the event, to not be resolved at all.
To catch errors where the field is missing from the event, it may be advisable to add a "default" engine type
and handle it in the configuration, eg:
